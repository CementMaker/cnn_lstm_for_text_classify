...
def train_step(model_train, batch, sequence_length, label):
    feed_dict = {
        ...
        model_train.model.sequence_length: sequence_length,
...
def dev_step(model_train, batch, sequence_length, label):
    ...
    print("dev_test: step {}, loss {}, accuracy {}".format(step, loss, accuracy))
...
class textDynamicRnnTrain(object):
    def __init__(self):
        ...
        self.model = TextDynamicRNN.lstm(layer_sizes=[128],
                                         embedding_size=100,
                                         vocab_size=20005,
                                         rnn_size=10,
                                         num_classes=6,
        ...
        self.batches = get_batch(10, 400, self.feature.train_x, self.feature.train_y, self.feature.seq_len_train)
...
def main(model_train):
    for data in model_train.batches:
        ...
        train_step(model_train, x_train, seq_len, y_train)
        current_step = tf.train.global_step(model_train.sess, model_train.global_step)
        if current_step % 10 == 0:
            dev_step(model_train, model_train.feature.test_x, model_train.feature.seq_len_test, model_train.feature.test_y)
